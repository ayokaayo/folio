...
export const kallax: SideProject = {
  ...
  mission: {
    ...
    spark: 'I DJ with vinyl. Every time I prepared for a set, I juggled three disconnected tools: 1. Discogs for my collection metadata, 2. manual notes and stickies for tracking BPMs, and 3. my own memory for records and tracks. I asked myself many times: "Why is there not one tool that understands how vinyl DJs actually work?" Not streaming DJs scrolling through infinite playlists or amateurs without real crowd pressure. Vinyl collectors who perform live, often without internet, in poorly lit booths, surrounded by dozens of physical records they need to locate quickly.\n\nThe name came naturally: Kallax. After the ever-present IKEA shelving unit that has become the unofficial furniture of record collectors worldwide. Modular, unpretentious, functional. The objective was clear:',

    intent: [
      'Build a companion app that works the way vinyl DJs do. Sync collection data from Discogs, fetch BPM information where it exists, and make manual entry effortless where it doesn\'t.',
  ...
  // ─────────────────────────────────────────────────────────────
  context: {
    background: 'The tools available to vinyl DJs exist in isolated silos. Discogs serves as the ubiquitous repository for collection metadata, but offers no DJ functionality. Mainstream software solutions like Rekordbox are made to work with digital files, not physical objects. Streaming platforms become irrelevant the moment you are standing in front of a crate, and then there is the BPM problem... Unlike digital DJs who have tempo metadata embedded in their files, vinyl DJs need to know or feel what speed their tracks are. Some pay services to analyze tracks they do not even own digitally. Others manually tap out tempos on calculators, but most just guess and adjust by ear.',
